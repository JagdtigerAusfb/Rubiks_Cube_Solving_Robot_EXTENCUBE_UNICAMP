"""M2/OP blind-solving method wrapper."""

import json
import logging

from solver.utils import build_result, count_moves, to_robot_sequence, invert_moves

logger = logging.getLogger(__name__)


def solve_from_file(json_path: str) -> dict:
    """Loads cube state from *json_path* and solves it with the M2/OP method.
    ...
    try:
        ...
        logger.info("Solving with M2/OP — state: %s", state)
        result = _solver_m2op(state)
        logger.info("M2/OP solution (%s moves): %s", result.get("move_count"), result.get("solution"))
        return result
